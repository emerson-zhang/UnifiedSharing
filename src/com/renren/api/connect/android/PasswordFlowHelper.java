...
public class PasswordFlowHelper {
				...
				if (obj != null) {
					if (!TextUtils.isEmpty(obj.optString("error"))) {
						...
						throw new RenrenException(
								RenrenError.ERROR_CODE_AUTH_FAILED, message,
								message);
					}
				}
				PasswordFlowResponseBean passwordFlow = new PasswordFlowResponseBean(
						response);
				return passwordFlow;
			} else {
				Util.logger("null response");
				throw new RenrenException(RenrenError.ERROR_CODE_UNKNOWN_ERROR,
			...
			public void run() {
				...
				try {
					...
					if (passwordFlow != null) {
						...
						values.putString(
								PasswordFlowResponseBean.KEY_ACCESS_TOKEN,
								passwordFlow.getAccessToken());
						values.putLong(PasswordFlowResponseBean.KEY_EXPIRES_IN,
								passwordFlow.getExpire());
						values.putString(
								PasswordFlowResponseBean.KEY_REFRESH_TOKEN,
								passwordFlow.getRefreshToken());
						values.putString(PasswordFlowResponseBean.KEY_SCOPE,
								passwordFlow.getScope());
						...
						Util.logger("null response");
						throw new RenrenException(
								RenrenError.ERROR_CODE_UNKNOWN_ERROR,
								"null response", "null response");
					}
				} catch (RenrenException e) {
					Util.logger("renren exception " + e.getMessage());
					if (listener != null) {
						listener.onRenrenAuthError(new RenrenAuthError(String
								.valueOf(e.getErrorCode()), e.getOrgResponse(),
								PASSWORD_FLOW_URL));
					}
					e.printStackTrace();
				} catch (Throwable e) {
					...
					if (listener != null) {
						listener.onRenrenAuthError(new RenrenAuthError(
								"on falut", e.getMessage(), PASSWORD_FLOW_URL));
					}
					e.printStackTrace();
				}
	...
	public void login(final Activity activity, PasswordFlowRequestParam param,
		...
		LoginEntryDialog dialog = new LoginEntryDialog(activity, param,
				listener, this, renren);
		dialog.show();
	...
	class LoginEntryDialog extends Dialog {
		...
		private void initViews(LinearLayout loginEntryView) {
			...
			Button button = (Button) loginEntryView
					.findViewById(R.id.renren_sdk_login_confirm_button);
