...
public class StatusPubActivity extends AbstractRenrenRequestActivity {
	...
	protected Dialog onCreateDialog(int id) {
		...
		case PROGRESS_DIALOG:
			...
			progressDialog.setMessage(this
					.getString(R.string.renren_sdk_publish_status_hint));
	...
	private void initStatus() {
		...
		if (intent.hasExtra(StatusSetRequestParam.STATUS_LABEL)) {
			status = intent
					.getParcelableExtra(StatusSetRequestParam.STATUS_LABEL);
		} else {
			Bundle bundle = intent.getExtras();
			if (bundle != null
					&& bundle.containsKey(StatusSetRequestParam.STATUS_LABEL)) {
				status = bundle
						.getParcelable(StatusSetRequestParam.STATUS_LABEL);
			} else {
	...
	private void initComponents() {
		...
		statusEdit.addTextChangedListener(new TextWatcher() {
			...
			public void onTextChanged(CharSequence s, int start, int before,
				...
				chCounterText.setText(status.getStatus().length() + "/"
						+ StatusSetRequestParam.MAX_LENGTH);
			}
		...
		publishButton.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View v) {
				if (status == null || status.getStatus() == null
						|| status.getStatus().trim().length() == 0) {
					showToast(StatusPubActivity.this
							.getString(R.string.renren_sdk_publish_null_status_alert));
				} else {
					publish();
				}
			}
	...
	private void publish() {
		...
		if (status.getStatus() != null
				&& status.getStatus().length() > StatusSetRequestParam.MAX_LENGTH) {
			status = status.trim();
		}

		aRr.publishStatus(status,
	...
	private void showToast(final String message) {
		handler.post(new Runnable() {
			@Override
			public void run() {
				Toast.makeText(StatusPubActivity.this, message,
